...
/**
 * Deterministic mutation suite derived from the successful v4 local/public vectors. Every
...
class PasetoMutationTest {
  ...
  private static final Map<String, EdECPublicKey> PUBLIC_KEYS = new HashMap<>();
  ...
  private static void setUpLocal() throws IOException {
    List<TestVectors.TestVector> vectors =
        TestVectors.v4(Purpose.PURPOSE_LOCAL).stream()
    ...
    LOCAL_KEYS.put("v4.local-unicode", key);
    LOCAL_MUTATIONS.addAll(
        TokenMutations.forLocalVector(
            "v4.local-unicode", unicodeToken, "fötter-フッター", "アサーション",
            NONCE_BYTES, TAG_BYTES, true));

    String minimalToken = PasetoLocal.encrypt(key, nonce, "", "", "");
    LOCAL_KEYS.put("v4.local-minimal", key);
    LOCAL_MUTATIONS.addAll(
        TokenMutations.forLocalVector(
            "v4.local-minimal", minimalToken, "", "", NONCE_BYTES, TAG_BYTES, true));
  }

  private static void setUpPublic() throws IOException {
    List<TestVectors.TestVector> vectors =
        TestVectors.v4(Purpose.PURPOSE_PUBLIC).stream()
            .filter(vector -> !vector.expectFail)
            .toList();
    for (TestVectors.TestVector vector : vectors) {
      PUBLIC_KEYS.put(vector.name, readPublicKey(vector.publicKeyPem));
    ...
    EdECPrivateKey privateKey = readPrivateKey(first.secretKeyPem);
    EdECPublicKey publicKey = readPublicKey(first.publicKeyPem);
    String unicodeToken = Paseto.sign(privateKey, "こんにちは 🌍 é", "fötter-フッター", "アサーション");
    PUBLIC_KEYS.put("v4.public-unicode", publicKey);
    PUBLIC_MUTATIONS.addAll(
        TokenMutations.forPublicVector(
            "v4.public-unicode", unicodeToken, "fötter-フッター", "アサーション",
            SIGNATURE_BYTES, true));

    String minimalToken = Paseto.sign(privateKey, "", "", "");
    PUBLIC_KEYS.put("v4.public-minimal", publicKey);
    PUBLIC_MUTATIONS.addAll(
        TokenMutations.forPublicVector(
            "v4.public-minimal", minimalToken, "", "", SIGNATURE_BYTES, true));
  }

  private static EdECPrivateKey readPrivateKey(String pem) throws IOException {
    try (Reader reader = new StringReader(pem)) {
      Object parsed = new PEMParser(reader).readObject();
      return (EdECPrivateKey) new JcaPEMKeyConverter().getPrivateKey((PrivateKeyInfo) parsed);
    }
  }

  private static EdECPublicKey readPublicKey(String pem) throws IOException {
    try (Reader reader = new StringReader(pem)) {
      Object parsed = new PEMParser(reader).readObject();
      return (EdECPublicKey) new JcaPEMKeyConverter().getPublicKey((SubjectPublicKeyInfo) parsed);
    }
  }
  ...
  void publicMutationIsRejected(String display, TokenMutation mutation) {
    EdECPublicKey key = PUBLIC_KEYS.get(mutation.vectorId());
    assertThrows(
        mutation.expectedException(),
  ...
  void everyStageIsCovered() {
    ...
    coverage.forEach(
        (stage, count) ->
            System.out.println("[v4] stage " + stage + " covered by " + count + " mutations"));
  }
}
